...
<<<<<<< HEAD
  ...
  // ################################### Submit URL Button Click #################################
    $(document).on("click", "#submit-url-button", function() {
      ...
      var articleUrl=$("input").val();
      $("#textinput").val("");
      analyzeArticle(articleUrl);
    });
      //############################# SMMRY API (Sumarize/ Title) ############################################
    //   var queryUrl = "https://cors-anywhere.herokuapp.com/" + "api.smmry.com/SM_API_KEY=CB55D94259&SM_URL=" + articleUrl + "&SM_IGNORE_LENGTH";
    //   $.ajax({
    //     url: queryUrl,
    //     method: "GET",
    //   }).then(function(response) {
...
//############################# Repeat Code for topics buttons (summarize) ###########################
    $(document).on("click", "#headline-button", function() {
      ...
      var articleUrl = $(this).attr("input");
      analyzeArticle(articleUrl);
    });
      // $.post(
      //   'https://apiv2.indico.io/summarization',
